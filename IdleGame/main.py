...
class MainGame(tk.Frame):
    ...
    class mainTabs(ttk.Notebook):
        def __init__(self, parent):
            ...
            self.tab2.columnconfigure(2, weight = 2)
            self.tab2.columnconfigure(3, weight = 1)
            self.tab2.rowconfigure(0, weight = 100)
            self.tab2.grid_propagate(False)
            
            # begin inventory layout
            self.invcanframe = tk.Frame(self.tab2)
            self.invcanframe.configure(background="#888888")
            self.invcanframe.grid_rowconfigure(0, weight = 1)
            self.invcanframe.grid_propagate(False)
            self.invcanframe.grid(row = 0, column = 1, sticky="nsew")

            self.invcanvas = tk.Canvas(self.invcanframe, background = "#888888")
            self.invcanvas.grid(row = 0, column = 0, sticky="nsew")

            bar = tk.Scrollbar(self.invcanframe, orient="vertical")
            bar.grid(row = 0, column = 1, sticky="nse")
            self.invcanvas.configure(yscrollcommand = bar.set)

            self.invboxes = tk.Frame(self.invcanvas)
            self.invboxes.configure(background="#888888")
            self.invboxes.grid(row = 0, column = 0, sticky="nsew")
            self.invboxes.grid_columnconfigure(0, weight = 1, uniform = "invgrid")
            self.invboxes.grid_columnconfigure(1, weight = 1, uniform = "invgrid")
            self.invboxes.grid_columnconfigure(2, weight = 1, uniform = "invgrid")
            self.invboxes.grid_columnconfigure(3, weight = 1, uniform = "invgrid")
            

            # end inventory layout, begin equipment layout
                   
            equipmentframe = i.EquipmentFrame(self.tab2)
            equipmentframe.grid(row = 0, column = 2, sticky = 'nsew')
        ...
        def lazyinit(self):
            i.itemBox.lazyinit(self)
            self.drawinventory()
            i.EquipmentFrame.lazyinit(self)
        ...
        def drawinventory(self):
            ...
            for k in g.player['inventory'].keys():
                ...
                b = i.itemBox(self.invboxes, k)

                if y < 4:
                    b.grid(row = x, column = y)
                    print(f"box at row {x} column {y}")
                    y += 1
